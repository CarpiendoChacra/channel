...
from TotalSecurity.utils.lang import *
...
from TotalSecurity.plugins import ALL_MODULES
from TotalSecurity.utils import paginate_modules
from lang import get_command
from TotalSecurity.utils.lang import (
    language,
    languageCB
)
from TotalSecurity.utils.start import (
    get_private_rules,
    get_learn
)
from TotalSecurity.mongo.usersdb import (
    adds_served_user,
    add_served_user
)
from config import var
from TotalSecurity.mongo.chatsdb import add_served_chat
...
async def start_bot():
    ...
    for module in ALL_MODULES:
        imported_module = importlib.import_module("TotalSecurity.plugins." + module)
        if (
    ...
    for i in ALL_MODULES:
        if j == 1:
            all_module += "•≫ Importado correctamente:{:<15}.py\n".format(i)
            j = 0
        else:
            all_module += "•≫ Importado correctamente:{:<15}.py".format(i)
        j += 1   
...
|                                             |  
|          Creado por BotNovedades            |  
|         (C) 2023                            |  
|         Que disfrutes del Bot :)            |
|_____________________________________________| """)
    await idle()
    await aiohttpsession.close()
    print("Detener a los clientes")
    await app.stop()
