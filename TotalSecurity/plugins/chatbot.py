...
from TotalSecurity import app,BOT_ID
from TotalSecurity.utils.filter_groups import cbot
from lang import get_command
from TotalSecurity.utils.lang import language
from TotalSecurity.mongo import chatb
from TotalSecurity.plugins.antlangs import get_arg
from TotalSecurity.utils.custom_filters import admin_filter
...
async def szcbot(_, message: Message):
    ...
    if chat:
        ...
        try:
           ...
           affiliateplus = requests.get(f"https://api.affiliateplus.xyz/api/chatbot?message={text}&botname=TotalSecurity&ownername=@supunma&user={chat_id}")
           textmsg = (affiliateplus.json()["message"])
           if "Affiliate+" in textmsg:
               textmsg = textmsg.replace("Affiliate+", "TotalSecurity")
           ...
           trtoen = (message.text if lang=="en" else tr.translate(message.text, dest="en").text).replace(" ", "%20")
           text = trtoen.replace(" ", "%20") if len(message.text) < 2 else trtoen
           safeone = requests.get(f"https://api.safone.tech/chatbot?message={text}&bot_name=TotalSecurity&bot_master=Supun&user_id={user_id}")
